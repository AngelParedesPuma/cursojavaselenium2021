...
//	}
	...
	public void loginOrange(String username, String pwd, boolean isLogged) {
		...
		type(txtUsername, username);
		type(txtPassword, pwd);
		takeScreenshot("Ingreso del user y el Pwd");
		click(btnLogin);
		takeScreenshot("click login");
		if(isLogged) {
			waitForElementPresent(btnWelcome);
			takeScreenshot("Validar login");
		}
	}
...
//	}
	...
	public void validateLoginFailed(String expectedMsg) {
		reporter("Validación de login invalido");
		...
		takeScreenshot("Captura del mensaje");
	}
